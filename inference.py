...
parser.add_argument("--lora", type=str, default="", help="activer un modèle lora il faut que ton modèle soit entrainé avec lora de base")
args = parser.parse_args()

model = torch.load("best_model.pt", weights_only=False)
if args.lora != "":
      lora_state_dict = torch.load(args.lora, map_location=device)

      # 3️⃣ Appliquer uniquement les poids LoRA
      model_state_dict = model.state_dict()
      for name, param in lora_state_dict.items():
            if name in model_state_dict:
                  model_state_dict[name].copy_(param)
            else:
                  print(f"[Attention] Clé LoRA non trouvée dans le modèle : {name}")
