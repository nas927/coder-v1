...
def launch_training(model, optimizer, scheduler, tokenizer):
    ...
    for epochs in range(args.epochs):
        ...
        for index_batch, batch in enumerate(batches):
            ...
            print(Fore.BLUE + "Taille des séquences du batch: ", len(batch[0].ids), Style.RESET_ALL)
            input_tensor: torch.Tensor = torch.as_tensor(batch['input_ids']).detach().clone().to(device)
